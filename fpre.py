...
def title_encode(title):
    title_words = [word_set.index(word) for word in title.split()]
    title_vector = np.zeros(15)
    title_index = np.zeros(15)
    title_vector[:len(title_words)] = title_words
    for i in range(len(title_words)):
        title_index[i]=1
    return title_vector,title_index
...
for i in range(1, movie_count + 1):
	...
	#new_v.append(Year)
    title_vector, title_index = title_encode(Title)
    new_i.extend(title_vector)
    new_v.extend(title_index) 
...
print("The number of movie is:", len(movie_i))
...
i1 = X_i_tr[:, 0:4]#user_feature
i2 = X_i_tr[:, 4:10]#genres
i5 = X_i_tr[:, 10:25]#title
i3 = X_i_tr[:, 25:]#year and timestamp
x1 = X_v_tr[:, 0:4]
x2 = X_v_tr[:, 4:10]
x5 = X_v_tr[:, 10:25]
x3 = X_v_tr[:, 25:]
...
np.save("train_i_title.npy", i5)
np.save("train_i_genre.npy", i2)
np.save("train_i_other.npy", i4)
np.save("train_x_title.npy", x5)
...
np.save("valid_i_title.npy", i5)
np.save("valid_i_genre.npy", i2)
np.save("valid_i_other.npy", i4)
np.save("valid_x_title.npy", x5)
...
np.save("test_i_title.npy", i5)
np.save("test_i_genre.npy", i2)
np.save("test_i_other.npy", i4)
np.save("test_x_title.npy", x5)
np.save("test_x_genre.npy", x2)
np.save("test_x_other.npy", x4)
np.save("test_y.npy", np.array(Y_te))


print('the number of train',len(X_i_tr))
print('the number of valid',len(X_i_va))
print('the number of test',len(X_i_te))
print('the number of Y',len(Y))
